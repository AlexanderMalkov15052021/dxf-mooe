...
export const setLayerSize = (mooeDoc: MooeDoc, layer: any, origin: Coords) => {
    ...
    mooeDoc.mSceneMap.mMapAttr.mMapOrigin.x = (layer?.vertices[0].x + origin.x) * scaleCorrection;
    mooeDoc.mSceneMap.mMapAttr.mMapOrigin.y = (layer?.vertices[0].y + origin.y) * scaleCorrection;

    mooeDoc.mSceneMap.mGridMsg.info.origin.position.x = (layer?.vertices[0].x + origin.x) * scaleCorrection;
    mooeDoc.mSceneMap.mGridMsg.info.origin.position.y = (layer?.vertices[0].y + origin.y) * scaleCorrection;
